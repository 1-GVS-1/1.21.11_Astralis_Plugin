...
public class EffectsRelics {
    ...
    public static void playAdaptEffects(Player player) {
        ...
        player.getWorld().spawnParticle(Particle.ELECTRIC_SPARK,
                player.getLocation(), 25, 0.5, 1, 0.5, 0.2);
        player.getWorld().spawnParticle(Particle.CLOUD,
                player.getLocation(), 15, 0.4, 0.8, 0.4, 0.02);
        player.getWorld().playSound(player.getLocation(), Sound.BLOCK_BEACON_POWER_SELECT, 0.6f, 1.2f);
    }
}
